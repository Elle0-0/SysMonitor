...
# Dash app as a sub-app of Flask
dash_app = Dash(__name__, server=app, url_base_pathname='/dashboard/')
dash_app.title = "SysMonitor Dashboard"
...
dash_app.layout = html.Div([
    ...
    dcc.Tabs([
        dcc.Tab(label='Device Metrics', children=[
            html.Div([dcc.Graph(id='cpu-usage-graph'), dcc.Graph(id='ram-usage-graph')])
        ]),
        dcc.Tab(label='Third Party Metrics', children=[
            html.Div([dcc.Dropdown(id='metric-dropdown', options=[
                {'label': 'Temperature', 'value': 'temp'},
                {'label': 'Humidity', 'value': 'humidity'}
            ], value='temp'), 
            dcc.Graph(id='weather-map', style={'height': '600px'})])
...
# Callback for updating Device Metrics
@dash_app.callback(
    Output('cpu-usage-graph', 'figure'),
    Output('ram-usage-graph', 'figure'),
    Input('interval-component', 'n_intervals')
)
def update_device_metrics(n_intervals):
    metrics = requests.get('https://michellevaz.pythonanywhere.com/api/metrics').json()
    device_metrics = metrics['device_metrics']
    
    # Filter data for CPU and RAM
    cpu_data = [m for m in device_metrics if m['metric_id'] == 'cpu_usage']
    ram_data = [m for m in device_metrics if m['metric_id'] == 'ram_usage']
    
    cpu_fig = go.Figure(data=[go.Scatter(x=[m['timestamp'] for m in cpu_data], y=[m['value'] for m in cpu_data], mode='lines+markers')])
    ram_fig = go.Figure(data=[go.Scatter(x=[m['timestamp'] for m in ram_data], y=[m['value'] for m in ram_data], mode='lines+markers')])

    return cpu_fig, ram_fig

# Callback for updating Third Party Metrics Map
@dash_app.callback(
    Output('weather-map', 'figure'),
    Input('interval-component', 'n_intervals'),
    Input('metric-dropdown', 'value')
)
def update_weather_map(n_intervals, selected_metric):
    metrics = requests.get('https://michellevaz.pythonanywhere.com/api/metrics').json()
    third_party_metrics = metrics['third_party_metrics']
    
    # Filter third-party metrics by selected type
    filtered_metrics = [m for m in third_party_metrics if m['name'] == selected_metric]
    
    weather_map_fig = go.Figure(data=[go.Scattergeo(
        lon=[m['longitude'] for m in filtered_metrics],
        lat=[m['latitude'] for m in filtered_metrics],
        text=[f"{m['name']}: {m['value']}" for m in filtered_metrics],
        mode='markers',
